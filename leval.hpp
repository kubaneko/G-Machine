...
auto
operator+ (body_t &&a, body_t &&b) -> function_prototype
{
  return { _int::ADD_M,
           { std::forward<body_t> (a), std::forward<body_t> (b) } };
}
...
auto
initialize_fun_lib () -> fun_lib
{
  fun_lib lib = {};
  std::vector<_int::GmInstr> binop
      = { { _int::UNWIND, std::monostate () },
          { _int::SLIDE, 3 },
          { _int::ADD, std::monostate () },
          { _int::EVAL, std::monostate () },
          { _int::PUSH, std::pair (_int::ARG, 2) },
          { _int::EVAL, std::monostate () },
  ...
  std::vector<_int::GmInstr> if_code
      = { { _int::UNWIND, std::monostate () },
          { _int::SLIDE, 4 },
          { _int::COND, std::pair (
                            std::vector<_int::GmInstr>{
                                { _int::PUSH, std::pair (_int::ARG, 1) } },
                            std::vector<_int::GmInstr>{
                                { _int::PUSH, std::pair (_int::ARG, 2) } }) },
          { _int::EVAL, std::monostate () },
          { _int::PUSH, std::pair (_int::ARG, 0) } };
...
class LEfunction
{
  ...
  void
  compile_helper (body_t body, int &pushed)
  {
    switch (body.index ())
      {
      case _int::GLOB:
      ...
      case _int::VALUE:
        {
          code.push_back (
              { _int::PUSH,
                std::pair (_int::VALUE, std::get<_int::VALUE> (body)) });
          ++pushed;
        }
        break;
      case _int::ARG:
        {
          code.push_back (
              { _int::PUSH,
                std::pair (_int::ARG,
                           std::get<_int::ARG> (body).arg + pushed) });
          ++pushed;
        }
        break;
      }
...
public:
  ...
  auto
  exec (const std::vector<int> &args) -> int
  {
    ...
    _int::G_machine g
        = { globals, _int::stack_i{}, _int::dump_t{}, std::move (init) };
    return g.eval ();
  }

  auto
  operator() (Arguments arguments) -> function_prototype
  {
